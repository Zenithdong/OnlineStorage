...
仓库根目录有三份规划文档，描述了**尚未落地**的改造方向。当前代码仍是 Windows/Qt/Winsock/qmake 实现，动手前先读对应文档：

- `MODERN_CPP_PLAN.md` — C++17 现代化改造（智能指针、Meyers 单例、`std::thread`、`reinterpret_cast`、MD5 定宽类型等），分阶段执行，每阶段保持可编译。
- `MIGRATION_PLAN.md` — Windows → Linux 移植（**保留 Qt**，Winsock→BSD socket、`CreateThread`→`std::thread`、MSVC CRT 函数→标准 C/POSIX、`.pro` pkg-config 化）。
- `Server/LINUX_REACTOR_PLAN.md` — 服务器 Linux 化 + **彻底去 Qt** + 单线程 epoll Reactor 重写（C++14，仅服务器端，客户端保持 Windows Qt）。与 `MIGRATION_PLAN.md` 在服务器端重叠且更激进，二者择一需先与用户确认。

三者共同的最优先级风险：`packdef.h` 中 3 处 `long` 字段（`STRU_UPLOADFILECONTENT_RQ::m_fileNum`、`STRU_SELECTFILE_RS::szFileNum`、`STRU_DOWNLOADFILE_RS::m_fileNum`）在 Windows(LLP64) 为 4 字节、Linux x86-64(LP64) 为 8 字节，跨平台会破坏二进制协议，须两端同步改为 `int32_t`。当前仍是 `long`，**尚未修改**。
...
数据库连接参数在 `Server/Kernel/kernel.cpp:open()` 中（地址 `127.0.0.1`、用户 `root`、库 `server`；密码经环境变量 `MYSQL_PASSWORD` 提供）。
...
- 代码风格由根目录 `.clang-format` 约束（LLVM 基、C++17、4 空格缩进、120 列、指针/引用左对齐）
- 根目录另有 `AGENTS.md`，内容与本文件高度重叠；修改开发约定时须同步更新二者。`Client/CLAUDE.md` 中 Qt 版本标注为 5.15.2，已过时，实际为 6.11.0
